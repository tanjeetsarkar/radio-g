...
class TestNewsFetcherInitialization:
    ...
    def test_fetcher_initialization_with_dedup(self, temp_config_file, check_redis):
        ...
        fetcher = NewsFetcher(config_path=str(temp_config_file), use_deduplication=True)
    ...
    def test_fetcher_initialization_without_dedup(self, temp_config_file):
        ...
        fetcher = NewsFetcher(
            config_path=str(temp_config_file), use_deduplication=False
        )
...
class TestNewsFetcherParsing:
    ...
    def test_parse_entry(self, temp_config_file, sample_rss_entry):
        ...
        with patch.object(fetcher.scraper, "scrape_article") as mock_scrape:
            mock_scrape.return_value = (
                ...
                {"status": "scraped_newspaper"},
            )

            news_item, dlq_events = fetcher._parse_entry(
                sample_rss_entry, source="Test Source", category="technology"
            )
    ...
    def test_parse_entry_missing_fields(self, temp_config_file):
        ...
        # Entry missing title
        bad_entry = {"link": "https://example.com/test"}

        result, dlq = fetcher._parse_entry(bad_entry, "Source", "tech")
    ...
    def test_date_parsing(self, temp_config_file):
        ...
        for date_str in date_formats:
            ...
            with patch.object(fetcher.scraper, "scrape_article") as mock_scrape:
                ...
                news_item, dlq_events = fetcher._parse_entry(entry, "Source", "tech")
...
class TestNewsFetcherWithMocks:
    ...
    def test_fetch_feed_success(
        self, mock_parse, temp_config_file, mock_feedparser_response
    ):
        ...
        with patch.object(fetcher.scraper, "scrape_article") as mock_scrape:
            ...
            items, failures = fetcher._fetch_feed(
                "https://example.com/feed", "Test Source", "technology"
            )
...
class TestNewsFetcherWithDeduplication:
    ...
    def test_deduplication_filters_duplicates(
        self, mock_parse, temp_config_file, check_redis
    ):
        ...
        with patch.object(fetcher.scraper, "scrape_article") as mock_scrape:
            ...
            items_1, failures_1 = fetcher._fetch_feed(
                "https://example.com/feed", "Test Source", "technology"
            )
            assert failures_1 == []
            ...
            items_2, failures_2 = fetcher._fetch_feed(
                "https://example.com/feed", "Test Source", "technology"
            )
            assert failures_2 == []
...
class TestNewsFetcherRetryLogic:
    ...
    def test_retry_on_failure(self, mock_sleep, mock_parse, temp_config_file):
        ...
        feed_config = {"url": "https://example.com/feed", "name": "Test Feed"}
