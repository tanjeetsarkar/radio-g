...
class TestReplayDLQKafkaConfig:
    ...
    def test_uses_full_config_when_default(
        self, mock_producer, mock_consumer, mock_get_config
    ):
        ...
        assert (
            consumer_call_kwargs["bootstrap_servers"]
            == mock_config.kafka.connection_config
        )
        ...
        assert (
            producer_call_kwargs["bootstrap_servers"]
            == mock_config.kafka.connection_config
        )
    ...
    @patch("scripts.replay_dlq.NewsKafkaConsumer")
    @patch("scripts.replay_dlq.NewsKafkaProducer")
    def test_uses_full_config_when_matching_string(
        self, mock_producer, mock_consumer, mock_get_config
    ):
    ...
    @patch("scripts.replay_dlq.NewsKafkaConsumer")
    @patch("scripts.replay_dlq.NewsKafkaProducer")
    def test_uses_simple_string_when_different(
        self, mock_producer, mock_consumer, mock_get_config
    ):
