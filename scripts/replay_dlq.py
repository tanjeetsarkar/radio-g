...
def replay_messages(
    ...
    if kafka_bootstrap_servers:
        ...
        if kafka_bootstrap_servers == config.kafka.bootstrap_servers:
            # If it matches the string in config, prefer the full
            # connection dict (which includes auth)
            ...
            try:
                ...
                if isinstance(news_payload, dict) and "url" in news_payload:
                    try:
                        ...
                        if (
                            "published_date" in news_payload
                            and "fetched_at" in news_payload
                        ):
                        ...
                        if not dry_run:
                            ...
                            logger.info(
                                f"-> Replayed item {news_item.id} to {target_topic}"
                            )
                        else:
                            logger.info(
                                f"-> [DRY RUN] Would replay item {news_item.id}"
                            )
                    ...
                    logger.warning(
                        ...
                        f"Payload is not a valid NewsItem structure"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--topic",
        choices=["ingestion", "processing"],
        required=True,
        help="Which DLQ to replay",
    )
    ...
    config = get_config()
    parser.add_argument(
        "--kafka", default=config.kafka.bootstrap_servers, help="Kafka servers"
    )
